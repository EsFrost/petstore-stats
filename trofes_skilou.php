...
<html>
	...
	<body>
		...
		<ul class="nav nav-pills">
		  ...
		  <li role="presentation" class="active"><a href="trofes_skilou.php">Τροφές Σκύλου</a></li>
		...
		<?php
			...
			$result = $mysqli->query("SELECT tr_sk_id, id_sk, etos, minas, posotita, apli, prem, super_prem FROM trofes_skilou ORDER BY tr_sk_id ASC");

			if ($result -> num_rows > 0) {
			//output data of each row
				while ($row = $result->fetch_assoc()) {
					echo '<div class="col_c col-lg-1 col-md-1 col-sm-1 col-xs-1">'.$row["tr_sk_id"].'</div>'.'<div class="col_c col-lg-1 col-md-1 col-sm-1 col-xs-1">'. $row["id_sk"].'</div>'.'<div class="col_c col-lg-1 col-md-1 col-sm-1 col-xs-1">'. $row["etos"].'</div>'.'<div class="col_c col-lg-1 col-md-1 col-sm-1 col-xs-1">'. $row["minas"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'. $row["posotita"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'. $row["apli"].'</div>' .'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'. $row["prem"]. '</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'. $row["super_prem"]. '</div>';
					echo "<br>";
				}
			}
			else {
		...
		<form type="get">
			...
			<option value="0">Id (επεξεργασία μόνο)</option>
				...
				$var2='">';
				$var3='</option>';
					$res_edit = $mysqli->query("SELECT tr_sk_id FROM trofes_skilou");

					while ($row_edit = $res_edit->fetch_assoc()) {
						echo $var1.$row_edit["tr_sk_id"].$var2.$row_edit["tr_sk_id"].$var3;
					}
	...
	<div class="lines col-lg-12 col-md-12 col-sm-12 col-xs-12"><hr></div>
		...
		elseif (isset($_GET['edit_var'])) {
			...
			if ($edit_id != 0) {
				...
				//int final is the value of posotita from trofes_skilou

				$int_check = $mysqli -> query("SELECT posotita, apli, prem, super_prem FROM trofes_skilou WHERE tr_sk_id = $edit_id");
				...
				if (is_numeric($apli) && is_numeric($prem) && is_numeric($super_prem)) {
					...
					elseif ($sum < $int_final) { //if the sum is less the user also gets an alert
						...
						$query_edit = $mysqli -> query("UPDATE trofes_skilou SET Apli = $apli, Prem = $prem, Super_prem = $super_prem WHERE tr_sk_id = $edit_id");
				...
				elseif (empty($apli) && (is_numeric($prem) && is_numeric($super_prem))) {
					...
					elseif ($sum < $int_final) { //if the sum is less the user also gets an alert
						...
						$query_edit = $mysqli -> query("UPDATE trofes_skilou SET Prem = $prem, Super_prem = $super_prem WHERE tr_sk_id = $edit_id");
				...
				elseif (empty($prem) && (is_numeric($apli) && is_numeric($super_prem))) {
					...
					elseif ($sum < $int_final) { //if the sum is less the user also gets an alert
						...
						$query_edit = $mysqli -> query("UPDATE trofes_skilou SET Apli = $apli, Super_prem = $super_prem WHERE tr_sk_id = $edit_id");
				...
				elseif (empty($super_prem) && (is_numeric($apli) && is_numeric($prem))) {
					...
					elseif ($sum < $int_final) { //if the sum is less the user also gets an alert
						...
						$query_edit = $mysqli -> query("UPDATE trofes_skilou SET Apli = $apli, Prem = $prem WHERE tr_sk_id = $edit_id");
				...
				elseif (empty($apli) && empty($prem) && is_numeric($super_prem)) {
					...
					elseif ($sum < $int_final) { //if the sum is less the user also gets an alert
						...
						$query_edit = $mysqli -> query("UPDATE trofes_skilou SET Super_prem = $super_prem WHERE tr_sk_id = $edit_id");
				...
				elseif (empty($apli) && empty($super_prem) && is_numeric($prem)) {
					...
					elseif ($sum < $int_final) { //if the sum is less the user also gets an alert
						...
						$query_edit = $mysqli -> query("UPDATE trofes_skilou SET Prem = $prem WHERE tr_sk_id = $edit_id");
				...
				elseif (empty($prem) && empty($super_prem) && is_numeric($apli)) {
					...
					elseif ($sum < $int_final) { //if the sum is less the user also gets an alert
						...
						$query_edit = $mysqli -> query("UPDATE trofes_skilou SET Apli = $apli WHERE tr_sk_id = $edit_id");
