...
<html>
	...
	<body>
		...
		<ul class="nav nav-pills">
		  ...
		  <li role="presentation" class="active"><a href="skilos.php">Σκύλος</a></li>
		...
		<div class="container col-lg-12 col-md-12 col-sm-12 col-xs-12"> <!-- name should be row_c instead of lines but php REALLY DID NOT LIKE IT! -->
			<div class="lines col-lg-12 col-md-12 col-sm-12 col-xs-12"><h1>Σκύλος</h1></div>
		...
		<?php
		    ...
		    $result = $mysqli->query("SELECT sk_id, etos, minas, trofes, accessories, farmaka FROM skilos ORDER BY sk_id ASC");

		if ($result -> num_rows > 0) {
			//output data of each row
			while ($row = $result->fetch_assoc()) {
				echo '<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["sk_id"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'. $row["etos"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["minas"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["trofes"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["accessories"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["farmaka"].'</div>';
			}
		}
		else {
			...
			<form type="get">
				...
				<div class="col_c col-lg-4 col-md-4 col-sm-4 col-xs-4"><h4>Id: <select name="edit_id">
					<option value="0">Id (διαγρφή/επεξεργασία μόνο)</option>
						...
						$var2='">';
						$var3='</option>';
							$res_edit = $mysqli->query("SELECT sk_id FROM skilos");

							while ($row_edit = $res_edit->fetch_assoc()) {
								echo $var1.$row_edit["sk_id"].$var2.$row_edit["sk_id"].$var3;
							}
		...
		<?php
			...
			//addition of values to table skilos
			...
			if (isset($_GET['sub'])) { //if button is pressed
				...
				if (is_numeric($trofes) && is_numeric($accessories) && is_numeric($farmaka)){ //checks if values are numeric
					...
					$query_add = $mysqli -> query("INSERT INTO skilos (Etos, Minas, Trofes, Accessories, Farmaka) Values ($etos, $minas, $trofes, $accessories, $farmaka)"); //inserts values into table

					$query_check = $mysqli -> query("SELECT sk_id FROM skilos"); //used to add values to table trofes_skilou



					if ($query_check -> num_rows > 0) {
						while ($test_row = $query_check->fetch_assoc()) {
							//echo $test_row["sk_id"]." ";
							$array = array($test_row["sk_id"]);
						}
					}

					$val = end($array);

					$query_add_2 = $mysqli -> query("INSERT INTO trofes_skilou (id_sk, etos, minas, posotita) Values ($val, $etos, $minas, $trofes)");

					echo '<script> window.location=" '.$_SERVER['PHP_SELF'].' "; </script>';


					//header("location:skilos.php");
				}
			...
			elseif (isset($_GET['del_val'])) { //delete function
				...
				if ($sk_id != 0){
					...
					if (is_numeric($sk_id)) {
						$query_del = $mysqli -> query("DELETE FROM skilos WHERE Sk_Id = '$sk_id'");

						$query_del_2 = $mysqli ->query("DELETE FROM trofes_skilou WHERE id_sk = '$sk_id'");
			...
			elseif (isset($_GET['edit_var'])) { //editi function
				...
				if ($edit_id != 0){
					if (is_numeric($trofes_edit) && is_numeric($accessories_edit) && is_numeric($farmaka_edit)) {
						$query_edit = $mysqli -> query("UPDATE skilos SET Etos = '$etos_edit', Minas = '$minas_edit', Trofes = '$trofes_edit', Accessories = '$accessories_edit', Farmaka = '$farmaka_edit' WHERE Sk_Id = '$edit_id'");

						$query_edit_1 = $mysqli -> query("UPDATE trofes_skilou SET posotita = '$trofes_edit' WHERE id_sk = '$edit_id'");
					...
					elseif (empty($trofes_edit) && (is_numeric($accessories_edit) && is_numeric($farmaka_edit))) {
						$query_edit = $mysqli -> query("UPDATE skilos SET Etos = '$etos_edit', Minas = '$minas_edit', Accessories = '$accessories_edit', Farmaka = '$farmaka_edit' WHERE Sk_Id = '$edit_id'");

						echo '<script> window.location=" '.$_SERVER['PHP_SELF'].' "; </script>';

						//eader("location:skilos.php");
					}
					elseif (empty($accessories_edit) && (is_numeric($trofes_edit) && is_numeric($farmaka_edit))) {
						$query_edit = $mysqli -> query("UPDATE skilos SET Etos = '$etos_edit', Minas = '$minas_edit', Trofes = '$trofes_edit', Farmaka = '$farmaka_edit' WHERE Sk_Id = '$edit_id'");
					...
					elseif (empty($farmaka_edit) && (is_numeric($trofes_edit) && is_numeric($accessories_edit))) {
						$query_edit = $mysqli -> query("UPDATE skilos SET Etos = '$etos_edit', Minas = '$minas_edit', Accessories = '$accessories_edit', Trofes = '$trofes_edit' WHERE Sk_Id = '$edit_id'");
					...
					elseif (empty($trofes_edit) && empty($accessories_edit) && is_numeric($farmaka_edit)) {
						$query_edit = $mysqli -> query("UPDATE skilos SET Etos = '$etos_edit', Minas = '$minas_edit', Farmaka = '$farmaka_edit' WHERE Sk_Id = '$edit_id'");

						echo '<script> window.location=" '.$_SERVER['PHP_SELF'].' "; </script>';
					}
					elseif (empty($trofes_edit) && empty($farmaka_edit) && is_numeric($accessories_edit)) {
						$query_edit = $mysqli -> query("UPDATE skilos SET Etos = '$etos_edit', Minas = '$minas_edit', Accessories = '$accessories_edit' WHERE Sk_Id = '$edit_id'");
					...
					elseif (empty($farmaka_edit) && empty($accessories_edit) && is_numeric($trofes_edit)) {
						$query_edit = $mysqli -> query("UPDATE skilos SET Etos = '$etos_edit', Minas = '$minas_edit', Trofes = '$trofes_edit' WHERE Sk_Id = '$edit_id'");
