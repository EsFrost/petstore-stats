...
<html>
	...
	<body>
		...
		<ul class="nav nav-pills">
		  ...
		  <li role="presentation" class="active"><a href="poulia.php">Πτηνά</a></li>
		...
		<div class="container col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="lines col-lg-12 col-md-12 col-sm-12 col-xs-12"><h1>Πτηνά</h1></div>
		...
		<?php
		    ...
		    $result = $mysqli->query("SELECT p_id, etos, minas, trofes, accessories, farmaka FROM poulia ORDER BY p_id ASC");

		if ($result -> num_rows > 0) {
			//output data of each row
			while ($row = $result->fetch_assoc()) {
				echo '<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["p_id"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'. $row["etos"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["minas"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["trofes"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["accessories"].'</div>'.'<div class="col_c col-lg-2 col-md-2 col-sm-2 col-xs-2">'.$row["farmaka"].'</div>';
			}
		}
		else {
			...
			<form type="get">
				...
				<div class="col_c col-lg-4 col-md-4 col-sm-4 col-xs-4"><h4>Id: <select name="edit_id">
					<option value="0">Id (διαγρφή/επεξεργασία μόνο)</option>
						...
						$var2='">';
						$var3='</option>';
							$res_edit = $mysqli->query("SELECT p_id FROM poulia");

							while ($row_edit = $res_edit->fetch_assoc()) {
								echo $var1.$row_edit["p_id"].$var2.$row_edit["p_id"].$var3;
							}
		...
		<?php
			...
			//addition of values to table poulia
			...
			if (isset($_GET['sub'])) { //if button is pressed
				...
				if (is_numeric($trofes) && is_numeric($accessories) && is_numeric($farmaka)){ //checks if values are numeric
					...
					$query_add = $mysqli -> query("INSERT INTO poulia (Etos, Minas, Trofes, Accessories, Farmaka) Values ($etos, $minas, $trofes, $accessories, $farmaka)"); //inserts values into table


					echo '<script> window.location=" '.$_SERVER['PHP_SELF'].' "; </script>';


					//header("location:poulia.php");
				}
			...
			elseif (isset($_GET['del_val'])) { //delete function
				...
				if ($p_id != 0){
					...
					if (is_numeric($p_id)) {
						$query_del = $mysqli -> query("DELETE FROM poulia WHERE P_Id = '$p_id'");
			...
			elseif (isset($_GET['edit_var'])) { //editi function
				...
				if ($edit_id != 0){
					if (is_numeric($trofes_edit) && is_numeric($accessories_edit) && is_numeric($farmaka_edit)) {
						$query_edit = $mysqli -> query("UPDATE poulia SET Etos = '$etos_edit', Minas = '$minas_edit', Trofes = '$trofes_edit', Accessories = '$accessories_edit', Farmaka = '$farmaka_edit' WHERE P_Id = '$edit_id'");
					...
					elseif (empty($trofes_edit) && (is_numeric($accessories_edit) && is_numeric($farmaka_edit))) {
						$query_edit = $mysqli -> query("UPDATE poulia SET Etos = '$etos_edit', Minas = '$minas_edit', Accessories = '$accessories_edit', Farmaka = '$farmaka_edit' WHERE P_Id = '$edit_id'");

						echo '<script> window.location=" '.$_SERVER['PHP_SELF'].' "; </script>';

						//eader("location:poulia.php");
					}
					elseif (empty($accessories_edit) && (is_numeric($trofes_edit) && is_numeric($farmaka_edit))) {
						$query_edit = $mysqli -> query("UPDATE poulia SET Etos = '$etos_edit', Minas = '$minas_edit', Trofes = '$trofes_edit', Farmaka = '$farmaka_edit' WHERE P_Id = '$edit_id'");
					...
					elseif (empty($farmaka_edit) && (is_numeric($trofes_edit) && is_numeric($accessories_edit))) {
						$query_edit = $mysqli -> query("UPDATE poulia SET Etos = '$etos_edit', Minas = '$minas_edit', Accessories = '$accessories_edit', Trofes = '$trofes_edit' WHERE P_Id = '$edit_id'");
					...
					elseif (empty($trofes_edit) && empty($accessories_edit) && is_numeric($farmaka_edit)) {
						$query_edit = $mysqli -> query("UPDATE poulia SET Etos = '$etos_edit', Minas = '$minas_edit', Farmaka = '$farmaka_edit' WHERE P_Id = '$edit_id'");

						echo '<script> window.location=" '.$_SERVER['PHP_SELF'].' "; </script>';
					}
					elseif (empty($trofes_edit) && empty($farmaka_edit) && is_numeric($accessories_edit)) {
						$query_edit = $mysqli -> query("UPDATE poulia SET Etos = '$etos_edit', Minas = '$minas_edit', Accessories = '$accessories_edit' WHERE P_Id = '$edit_id'");
					...
					elseif (empty($farmaka_edit) && empty($accessories_edit) && is_numeric($trofes_edit)) {
						$query_edit = $mysqli -> query("UPDATE poulia SET Etos = '$etos_edit', Minas = '$minas_edit', Trofes = '$trofes_edit' WHERE P_Id = '$edit_id'");
